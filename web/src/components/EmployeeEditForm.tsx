...
import { useUpdateEmployeeForm } from "../hooks/useEmployeeForm";
import { useEmployeeStore } from "../stores/employeeStore";
import { EmployeeFormFields } from "./EmployeeFormFields";
import { UpdateEmployeeFormData } from "../utils/employeeValidation";

export const EmployeeEditForm: React.FC = () => {
  const { editingEmployee, updateEmployee, setEditingEmployee } = useEmployeeStore();

  if (!editingEmployee) return null;

  const initialData: UpdateEmployeeFormData = {
    name: editingEmployee.name,
    email: editingEmployee.email,
    department: editingEmployee.department || "",
  };

  const { formData, errors, isSubmitting, handleChange, handleSubmit } = useUpdateEmployeeForm({
        ...
        <div className="liquid-form rounded-3xl p-8 max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h2 className="text-2xl font-bold text-slate-800/90">従業員情報編集</h2>
                <p className="text-slate-600/70 text-sm">従業員の情報を更新してください</p>
              </div>
            </div>
            <button
